...
#'
#' @param eplist is the list containing Exome sheared fragments and  corresponding primers achieved by 'List_PrimerExomeSet' function
#'
...
List_PairExomeSet<-function(eplist,dir="left"){
...
Result=list()
for (i in 1:length(eplist)) {#length(eplist)
  Result[[i]]<-List_PairSet(eplist[[i]])
}
